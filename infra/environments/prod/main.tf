...
module "app" {
  ...
  tags                = var.tags
  apps = {
    backend = { name = "${var.name}-api-prod", runtime = "NODE|18-lts" }
    frontend = { name = "${var.name}-web-prod", runtime = "NODE|18-lts" }
  }
}
